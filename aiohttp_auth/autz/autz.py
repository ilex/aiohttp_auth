...
def autz_middleware(autz_policy):
    """Return aiohttp_auth authorization middleware factory.
...
def setup(app, autz_policy):
    ...
    Note that aiohttp_auth.auth middleware should be installed too to use autz
    middleware. So the preferred way to install this middleware is to use
    global aiohttp_auth.setup function.
